...
- **commit and push** — Loomforge commits changes in the worktree and pushes the issue branch to remote
...
- permission mode: `--dangerously-bypass-approvals-and-sandbox` (no interactive prompts; the builder must write files, run commands, and iterate autonomously within the worktree)
...
5. **Gate** — include the project's verification commands so the builder self-checks
   before finishing. This avoids unnecessary revision loops by catching gate
   failures before Loomforge's separate verification phase.
...
4. **Verification evidence** — Loomforge must include verification command results
   (pass/fail per command) in the review prompt so the reviewer can assess
   whether checks are meaningful
...
- `verifying`: Loomforge is running configured checks
...
V1 integration should stay explicit. OpenClaw's role is simplified: it tells Loomforge *what* to work on (by issue identifier), and Loomforge handles the rest.
...
11. `launchd` installer/status integration for running `loomforged` with OpenClaw startup
