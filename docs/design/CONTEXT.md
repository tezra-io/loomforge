# Loomforge V1 Context

## What Loomforge is
Loomforge is a slim local workflow engine for software delivery. It exists to keep the parts of Paperclip that actually help, while avoiding the org-chart and platform overhead we do not need.
...
- selecting which issues enter Loomforge (by identifier only)
...
- small durable ready queue owned by Loomforge
- SQLite state store
...
- `loomforged`: long-lived local daemon, managed by `launchd`
- `loomforge` CLI: trigger/status/logs/retry
...
- Runtime data should live under `~/.loomforge/`
